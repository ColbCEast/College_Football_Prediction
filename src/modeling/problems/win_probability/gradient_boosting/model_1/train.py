"""
Gradient Boosting Model 1 — Win Probability

Trains and evaluates a baseline Gradient Boosting classifier for
...
Outputs:
    models/win_probability/gradient_boosting/model_1/
...
Notes:
    - No feature scaling is used because Gradient Boosting does not require it.
    ...
    - Model 1 uses the complete 310-feature predictor space.
    - No feature selection or temporal hyperparameter tuning is performed.
"""
...
# src/modeling/problems/win_probability/gradient_boosting/model_1/train.py
#
...
# Columns excluded from the Gradient Boosting predictor set.
#
# season:
...
#     and Random Forest predictor sets.
#
# win_home:
#     Target variable.
EXCLUDED_COLUMNS = [
...
def main():
    ...
    print()
    print("=" * 80)
    print("GRADIENT BOOSTING MODEL 1 — WIN PROBABILITY")
    print("=" * 80)
    ...
    for parameter, value in GB_PARAMS.items():
        print(f"  {parameter}: {value}")

    print()
    print("Feature space:")
    print("  - Complete predictive-safe feature set")
    ...
    print(
        ...
        "and training Gradient Boosting classifier..."
    )
    ...
    print(
        f"Complete model pipeline: {model_path}"
    )
    ...
    print(
        f"Saved feature list: {feature_list_path}"
    )
    ...
    config_df = pd.DataFrame({
        "parameter": list(GB_PARAMS.keys()),
        "value": [
            str(value)
            for value in GB_PARAMS.values()
        ],
    })
    ...
    print(
        f"Saved model configuration: {config_path}"
    )
    ...
    print()
    print("=" * 80)
    print("GRADIENT BOOSTING MODEL 1 COMPLETE")
    print("=" * 80)
